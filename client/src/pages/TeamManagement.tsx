    ...
    return (
        <div className="space-y-5 animate-fade-in pb-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-xs font-semibold text-violet-500">Team Management</span>
                    </div>
                    <p className="text-sm text-gray-400 mt-1 font-medium max-w-lg">
                        Manage your <span className="text-gray-700 font-semibold">team members</span> and control who has access to your dashboard.
                    </p>
                </div>
                <Button
                    onClick={() => setShowAddForm(true)}
                    className="h-10 px-5 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-600 text-white text-xs font-semibold shadow-lg shadow-violet-500/20 hover:shadow-violet-500/40 hover:-translate-y-0.5 transition-all duration-300"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                    { label: 'Total Members', value: team.length, icon: Users, color: 'violet' },
                    ...
                    <div key={s.label} className="glass-card p-5 flex items-center gap-4 group hover:shadow-lg transition-all duration-300">
                        <div className={`h-11 w-11 rounded-xl bg-gradient-to-br ${
                            s.color === 'violet' ? 'from-violet-500/15 to-indigo-500/15' :
                            s.color === 'emerald' ? 'from-emerald-500/15 to-green-500/15' :
            ...
            {showAddForm && (
                <div className="glass-card p-6 border-2 border-violet-200 space-y-5 animate-fade-in relative overflow-hidden">
                    {/* Decorative glow */}
                    <div className="absolute -top-10 -right-10 w-32 h-32 bg-violet-500/5 rounded-full blur-2xl" />
                    ...
                    <div className="flex items-center justify-between relative z-10">
                        ...
                        <button
                            ...
                            className="text-gray-300 hover:text-gray-500 transition-colors text-lg font-bold"
                        >
                            ×
                        </button>
                    </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    className="h-11 rounded-xl bg-gray-50 border-gray-200 pl-10 text-sm focus-visible:ring-violet-200"
                                    onKeyDown={(e) => e.key === 'Enter' && handleAddMember()}
                                />
                            </div>
                        </div>
                        <div className="space-y-2">
                            <Label className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Name (Optional)</Label>
                            <Input
                                value={newName}
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <Button
                            variant="outline"
                            onClick={() => { setShowAddForm(false); setNewEmail(''); setNewName('') }}
                            className="h-10 px-6 rounded-xl border-gray-200 text-xs font-semibold text-gray-500 hover:bg-gray-50"
                        >
                            Cancel
                        </Button>
                    </div>
                    ...
                    {/* Info Box */}
                    <div className="p-4 rounded-xl bg-violet-50/50 border border-violet-100 relative z-10">
                        <div className="flex items-start gap-2">
                            <AlertCircle className="h-4 w-4 text-violet-500 mt-0.5 shrink-0" />
                            <p className="text-[11px] text-gray-500 font-medium leading-relaxed">
                                The team member will need to <span className="text-violet-600 font-semibold">sign up</span> using this exact email address. 
            ...
            {team.length > 2 && (
                <div className="relative">
                    ...
                    <Input
                        ...
                        className="h-10 rounded-xl bg-white border-gray-200 pl-10 text-sm focus-visible:ring-violet-200"
                    />
                </div>
            )}
            ...
            <div className="glass-card overflow-hidden">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 px-4">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Invite your team to collaborate on this dashboard</p>
                        <Button
                            onClick={() => setShowAddForm(true)}
                            className="mt-4 h-9 px-5 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-600 text-white text-xs font-semibold shadow-lg shadow-violet-500/20"
                        >
                    ...
                    <div>
                        ...
                        {filteredTeam.map((member, idx) => (
                            <div
                                key={member.id + '-' + idx}
                                className="grid grid-cols-12 gap-4 px-6 py-4 items-center border-b border-gray-50 last:border-0 hover:bg-violet-50/30 transition-colors duration-200 group"
                            >
                                {/* Member Info */}
                                <div className="col-span-5 flex items-center gap-3">
                                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-md ${
                                        member.isOwner
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-sm font-semibold text-gray-800 truncate">
                                            {member.name || 'Unnamed'}
                                            {member.isOwner && (
                                                <span className="ml-2 text-[9px] font-bold text-violet-500 uppercase tracking-wider">You</span>
                                            )}
                                        </p>
                                ...
                                <div className="col-span-2">
                                    {member.isOwner ? (
                                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-50 text-violet-600 text-[10px] font-bold uppercase tracking-wider">
                                            <Crown className="h-3 w-3" />
                                            Owner
                                        </span>
                                    ) : (
                                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 text-blue-600 text-[10px] font-bold uppercase tracking-wider">
                                ...
                                <div className="col-span-2 flex justify-end">
                                    {!member.isOwner && (
                                        <Button
                                            ...
                                            className="h-8 w-8 p-0 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all duration-200"
                                            title="Remove member"
                                        >
            ...
            {/* Info Footer */}
            <div className="p-5 rounded-2xl bg-gradient-to-r from-violet-50/50 via-indigo-50/50 to-purple-50/50 border border-violet-100/60">
                <div className="flex items-start gap-3">
                    <div className="h-9 w-9 rounded-xl bg-violet-100/80 flex items-center justify-center shrink-0">
                        <UserCog className="h-4.5 w-4.5 text-violet-500" />
                    </div>
                    <div>
                        <h4 className="text-xs font-bold text-gray-700">How Team Access Works</h4>
                        <ul className="mt-2 space-y-1.5 text-[11px] text-gray-500 font-medium leading-relaxed">
                            ...
                            <li className="flex items-start gap-2">
                                ...
                                They sign up using the <span className="text-violet-600 font-semibold">same email</span> on the login page
                            </li>
                            <li className="flex items-start gap-2">
